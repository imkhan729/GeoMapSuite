...
export async function generateMetadata(props: { params: Promise<{ state: string }> }): Promise<Metadata> {
  ...
  const title = buildSeoTitle(`${stateData.name} Counties: Map & List`);
  const description = buildSeoDescription(`Explore ${stateData.name} counties with an interactive county map, county seats, population, and FIPS codes. Browse the complete county list by state.`);
