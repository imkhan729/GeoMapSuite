...
interface MapFaqAccordionProps {
  faqs: MapFaqItem[];
  mapName?: string;
  title?: string;
  subtitle?: string;
...
export function MapFaqAccordion({
  faqs,
  mapName = 'this map',
  title = `Frequently Asked Questions`,
  subtitle,
}: MapFaqAccordionProps) {
  // Normalize FAQs to { question, answer }
  ...
  const expandAll = () => setOpenIndices(normalizedFaqs.map((_, i) => i));
  const collapseAll = () => setOpenIndices([]);

  const defaultSubtitle = `Authoritative People Also Ask answers regarding printable blank maps of ${mapName}, administrative boundaries, vector editing, and commercial licensing.`;

  return (
    <section
      id="faq"
      aria-labelledby="map-faq-heading"
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-[#e8e6e1] pb-6">
        <div className="space-y-1.5 max-w-2xl">
          ...
          <h2
            id="map-faq-heading"
            className="text-2xl sm:text-3xl font-serif font-bold text-[#1a1a18] tracking-tight"
          >
            {title}
          </h2>
            ...
            >
              ...
              {isOpen && (
                <div
                  ...
                  className="px-5 pb-6 sm:px-6 sm:pb-6 pt-0 border-t border-[#f0eee8]/60 mt-1"
                >
