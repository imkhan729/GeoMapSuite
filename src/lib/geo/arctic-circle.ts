export const ARCTIC_CIRCLE_REFERENCE_LATITUDE = 66.5636;
const WGS84_SEMI_MAJOR_AXIS_METERS = 6_378_137;
const WGS84_FLATTENING = 1 / 298.257223563;

export function clampArcticLongitude(longitude: number): number {
  if (!Number.isFinite(longitude)) return 0;
  return Math.max(-180, Math.min(180, longitude));
}

export function formatArcticLongitudeDms(longitude: number): string {
  const bounded = clampArcticLongitude(longitude);
...
/** WGS 84 length of the approximate Arctic Circle reference parallel. */
export function getArcticCircleCircumferenceKm(latitude = ARCTIC_CIRCLE_REFERENCE_LATITUDE): number {
